...
AI-powered semantic code search using vector embeddings. Finds code by meaning, not just text patterns.
...
```typescript
{
  ...
  parameters: {
    ...
    min_score?: number;      // Similarity threshold 0-1 (default: 0.3)
    path_pattern?: string;   // Filter by file path substring
    file_types?: string[];   // Filter by extensions [".ts", ".py"]
    languages?: string[];    // Filter by language ["typescript", "python"]
    symbol_types?: string[]; // Filter by ["function", "class", "interface"]
    include_content?: boolean; // Include code content (default: true)
  }
}
```
...
// Find database connection handling in TypeScript
...
// Find React components for file upload
{ name: 'codebase_search', parameters: {
...
```typescript
const result = await window.vyotiq.semantic.indexWorkspace({
  forceReindex?: boolean;   // Rebuild entire index
  fileTypes?: string[];     // Specific extensions to index
...
```typescript
const result = await window.vyotiq.semantic.search(query: string, {
  limit?: number;           // Max results (default: 10)
  minScore?: number;        // Similarity threshold 0-1 (default: 0.3)
  filePathPattern?: string; // Filter by path substring
  fileTypes?: string[];     // Filter by extensions
  languages?: string[];     // Filter by language
  symbolTypes?: string[];   // Filter by symbol type
  includeContent?: boolean; // Include code content (default: true)
});
```
...
#### `semantic:getProgress`

Get current indexing progress.

```typescript
const progress = await window.vyotiq.semantic.getProgress();
```
...
```typescript
const stats = await window.vyotiq.semantic.getStats();
```
...
```typescript
window.vyotiq.semantic.onProgress((progress) => {
  console.log(`${progress.indexedFiles}/${progress.totalFiles} files indexed`);
});
```

#### `semantic:modelStatus`

Emitted when embedding model status changes.

```typescript
window.vyotiq.semantic.onModelStatus((status) => {
  console.log(`Model ${status.modelId}: ${status.status}`);
});
```
...
```typescript
window.vyotiq.semantic.onModelProgress((progress) => {
...
```typescript
import { useSemanticIndex } from '@/hooks';

function MyComponent() {
  const {
    progress,           // Current indexing progress
    stats,              // Index statistics
    isReady,            // Whether indexer is ready
    isLoadingStats,     // Whether loading stats
    indexWorkspace,     // Start indexing
    search,             // Perform search
    clearIndex,         // Clear index
    abortIndexing,      // Abort indexing
  ...
  // Search
  const results = await search('authentication logic', { limit: 5 });
}
```
