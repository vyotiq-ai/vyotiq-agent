...
### File Not Opening in Editor

**Symptoms:**
- File tree shows file but won't open
- Editor panel stays blank
...
1. **Check file permissions:**
   ...
   - Try opening different file

2. **Check file size:**
   - Very large files may not open
   - Try opening smaller file first
   - Check available memory

3. **Check file encoding:**
   - Verify file is UTF-8 encoded
   - Try converting file encoding
...
1. **Check file extension:**
   - Verify file has correct extension (.ts, .py, .js, etc.)
   - Rename file if extension is wrong

2. **Check language support:**
   - Monaco editor supports 50+ languages
   - Verify language is supported
   - Check documentation for supported languages
...
4. **Update Monaco:**
   - Check for updates: `npm update @monaco-editor/react`
   - Rebuild: `npm rebuild`

### Diagnostics Not Showing

**Symptoms:**
- No error/warning squiggles
...
1. **Enable diagnostics:**
   - Settings → Editor → Enable diagnostics
   - Restart editor

2. **Check LSP status:**
   - Settings → Advanced → LSP status
   - Verify LSP servers are running
   - Check for LSP errors in logs

3. **Restart LSP:**
   - Settings → Advanced → Restart LSP
...
**Symptoms:**
- `codebase_search` tool returns "index not ready"
- Semantic search not available

**Solutions:**

1. **Wait for indexing to complete:**
   - Check Settings → Indexing for progress
   - Initial indexing can take several minutes for large codebases

2. **Manually trigger indexing:**
   - Go to Settings → Indexing
   - Click "Reindex" button

3. **Check if indexing is enabled:**
   - Settings → Indexing → Enable Semantic Indexing
   - Enable "Auto-index on Startup"

### Slow Indexing Performance

**Symptoms:**
- Indexing takes extremely long
- High CPU usage during indexing

**Solutions:**

1. **Enable GPU acceleration:**
   - Settings → Indexing → Use GPU Acceleration
   - Requires compatible GPU

2. **Reduce indexed files:**
   - Add patterns to "Exclude Patterns"
   - Limit file types in "File Types"

3. **Increase chunk size:**
   - Settings → Indexing → Target Chunk Size
   - Larger chunks = fewer embeddings to generate

4. **Use "fast" quality preset:**
   - Settings → Indexing → Embedding Quality → Fast

### Poor Search Results

**Symptoms:**
- Search doesn't return relevant code
- Too few results returned

**Solutions:**

1. **Lower minimum score threshold:**
   - Settings → Indexing → Min Search Score (try 0.2 or lower)

2. **Use more descriptive queries:**
   - Instead of "auth" → "user authentication login logic"
   - Describe the functionality you're looking for

3. **Force reindex:**
   - Settings → Indexing → Reindex
   - Ensures all files are properly indexed

4. **Check file type filters:**
   - Make sure target file types are being indexed
...
3. **Clear model cache:**
   - Delete `~/.cache/huggingface/` directory
   - Restart and let models re-download

4. **Check disk space:**
   - Models require ~500MB of disk space

### High Memory Usage

**Symptoms:**
- App uses excessive memory during indexing
- System becomes slow

**Solutions:**

1. **Reduce cache size:**
   - Settings → Indexing → Max Cache Entries (try 5000)

2. **Disable GPU acceleration:**
   - GPU acceleration can use more memory

3. **Reduce max file size:**
   - Settings → Indexing → Max File Size

4. **Exclude large directories:**
   - Add `**/node_modules/**`, `**/build/**` to exclude patterns
