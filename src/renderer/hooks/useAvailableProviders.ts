 ...
 * 
 * Uses the shared IPC retry utility to handle cases where the backend
 * IPC handlers might not be fully registered when the hook first mounts.
 */
...
export function useAvailableProviders(): UseAvailableProvidersResult {
  ...
  const fetchProviders = useCallback(async () => {
    ...
    try {
      ...
      // Use IPC retry utility to handle race conditions with handler registration
      const [providers, cooldown] = await Promise.all([
        withIpcRetry(
          () => window.vyotiq.agent.getAvailableProviders(),
          { operationLabel: 'agent:get-available-providers', maxAttempts: 5, retryDelayMs: 300 }
        ),
        withIpcRetry(
          () => window.vyotiq.agent.getProvidersCooldown(),
          { operationLabel: 'agent:get-providers-cooldown', maxAttempts: 5, retryDelayMs: 300 }
        ),
      ]);
